...
function initTouch() {
  ...
  var pinch = null;      // 雙指縮放的起始狀態
  var shown = false;     // 指標目前是否顯示中
  var wasShown = false;  // 這次觸控開始時指標是否已顯示
  var moved = false;     // 這次觸控有沒有拖動（用來區分輕點與拖曳）
  var taken = false;     // 這次觸控是否由我們接管
  var sx = 0, sy = 0;
  var TAP_SLOP = 6;      // 位移小於這個距離視為輕點
  ...
  gd.addEventListener('touchend', function (e) {
    if (!taken) return;
    if (e.touches.length > 0) return;      // 還有手指按著
    e.stopPropagation();
    if (pinch) { pinch = null; taken = false; return; }
    // 原本就顯示著，而且這次只是輕點沒拖動 → 收起指標
